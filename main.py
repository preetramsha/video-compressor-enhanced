...
class Window(QWidget):
    def __init__(self) -> None:
        ...
        # File Size Label
        if "target_percentage" not in self.settings:
        ...
        self.label_size.move(SIZE_PERCENT_LABEL.x, SIZE_PERCENT_LABEL.y)

        # File Size Slider
        self.slider_size = QSlider(Qt.Orientation.Horizontal, self)
        self.slider_size.resize(SIZE_PERCENT_SLIDER.w, SIZE_PERCENT_SLIDER.h)
        self.slider_size.move(SIZE_PERCENT_SLIDER.x, SIZE_PERCENT_SLIDER.y)
        self.slider_size.setRange(1, 100)
        self.slider_size.setValue(self.settings["target_percentage"])
        self.slider_size.valueChanged.connect(self.update_size_label)
        ...
        # One Pass Label
        self.label_one_pass = QLabel("1-Pass", self)
        self.label_one_pass.resize(ONE_PASS_LABEL.w, ONE_PASS_LABEL.h)
        self.label_one_pass.move(ONE_PASS_LABEL.x, ONE_PASS_LABEL.y)

        # One Pass Checkbox
        self.checkbox_one_pass = QCheckBox(self)
        self.checkbox_one_pass.resize(ONE_PASS_CHECKBOX.w, ONE_PASS_CHECKBOX.h)
        ...
        self.label_one_pass.setStyleSheet(LABEL_STYLE)
    ...
    def update_size_label(self, value):
        self.label_size.setText(f"Size: {value}%")
    ...
    def compress_videos(self):
        ...
        self.compress_thread = CompressionThread(
            self.slider_size.value(), self.checkbox_gpu.isChecked(), self.checkbox_one_pass.isChecked()
        )
